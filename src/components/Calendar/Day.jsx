            ...
            return day[1].map((el, index) => {
                ...
                return (
                    <CustomTooltip title={tooltip} arrow key={`day_event_${el.event.name}_${index}`}>
                        <div className='day__events' style={el.event.class === 'Debit' ? { backgroundColor: 'var(--s__red__color)' } : (el.event.class === 'Credit' ? { backgroundColor: 'var(--s__green__color)' } : { backgroundColor: 'var(--p__orange__color)' })}
                        >
